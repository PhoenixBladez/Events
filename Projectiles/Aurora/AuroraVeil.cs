...
{
	public class AuroraVeil : ModProjectile
	{
		int target;
		...
		public override void AI()
		{
			...
			for (int j = 0; j < 6; j++)
			{
				...
				int num2 = Dust.NewDust(projectile.Center, 6, 6, d1, 0f, 0f, 0, default(Color), 1f);
			...
			if (index1 >= 0 && Main.player[index1].active && !Main.player[index1].dead && Main.netMode != 1)
			{
				if (projectile.Distance(Main.player[index1].Center) <= num34)
					return;
				Vector2 unitY = projectile.DirectionTo(Main.player[index1].Center);
				if (unitY.HasNaNs())
					unitY = Vector2.UnitY;
				projectile.velocity = (projectile.velocity * (num12 - 1f) + unitY * num23) / num12;
			}
			else
			{
